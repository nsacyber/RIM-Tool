...
- If present without any path specified (i.e., no additional string input present after the option), Support RIM validation will be enabled. Additionally, the validation will be performed from **absolute** file system paths that are specified in the base RIM. Ensure that these paths and associated Support RIM files are present on the file system, in this case.
- If present with a path specified after the option, Support RIM Validation will also be enabled. However, the validation will be performed by using the specified directory as a **working directory**, and therefore paths will be **relative** from that directory.
