...
of the TPM Event Log in C:\Windows\Logs\MeasuredBoot\ with a numeric name and a .log extension.
...
examples of events are POST_CODE, VARIABLE_DRIVER_CONFIG, EV_IPL, etc. While this is similar to 
the TPM Event Log, the big difference is that the TCG Event Log Assertion Support RIM is the 
expected data from the OEM, while the TPM Event Log is the measured data from a client PC.
...
1. Real-time values check: When the computer boots, UEFI creates the TPM Event Log (the UEFI 
“audit” log) which captures the firmware events and their corresponding digests. This Event Log 
is sent to the ACA. The ACA uses the digest values within the Event Log to calculate a composite 
